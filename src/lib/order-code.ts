// ============================================================
// Order code — the human-facing 5-char order id, e.g. "FA100".
//
...
//
// Codes for the same year sort chronologically as plain strings (fixed width),
// so lexicographic order == creation order.
// ============================================================
...
/** The ISO week-year (differs from the calendar year for a few days near Jan 1
 * / Dec 31). */
...
/** 52 or 53 — how many ISO weeks the given ISO year has (Dec 28 is always in
 * the last week). */
export function weeksInIsoYear(year: number): number {
  // Local noon Dec 28 — always in the ISO year's last week, and TZ-safe (its
  // local y/m/d, which isoWeek reads, is unambiguously Dec 28).
...
/** Next order code for `date`, given the codes already used (any is_training
 * scope). Fills the band from its base upward. */
...
export function codeLabel(d: { order_code?: string | null; order_no?: number; order_suffix?: string | null }): string {
