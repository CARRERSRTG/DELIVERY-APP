-- 020: Per-store auto-approval.
--
-- A store can be flagged auto_approve (Data page → Stores). Orders sold from
-- such a store are created already "approved" by whoever places them, skipping
-- the manager approval step. This teaches the stage guard to allow a
-- sales/driver to INSERT (or move a draft/pending order) straight to
-- 'approved' when the order's store auto-approves. Everything else matches 019.
-- Safe to re-run.

-- Reads settings.stores (jsonb array of {name, address, auto_approve}) and
-- returns true when the named store is flagged auto_approve.
