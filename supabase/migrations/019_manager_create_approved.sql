-- 019: Let Office Managers create an ALREADY-APPROVED order in one step.
--
-- 017 let managers insert, but only at draft/pending — yet the app's
-- "Create order (approved)" button (OrderModal) inserts a manager's order
-- directly at stage='approved'. So managers still hit
-- "New orders start as draft or pending". This widens the INSERT rule so a
-- manager may create at draft, pending OR approved; sales/drivers unchanged.
-- Everything else matches 017 (split-load + re-delivery inserts, the
-- stage-change edit rules, and the transition table). Safe to re-run.
